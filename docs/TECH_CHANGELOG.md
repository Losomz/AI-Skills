...
- `/git commit` 现在只接受可选的核心要求，不再解析 agent 参数；后台进程仍在真实 `ctx.cwd` 中提交和推送。`subagent-models.json` v1 文件会兼容读取，并在下一次 `/subagent` 保存时升级为 v2。更新 PiCraft package 或手工同步全局扩展后需执行 `/reload`。
