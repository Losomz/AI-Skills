...
4. Report conflicts, empty changes, or commit failures through a UI-only notification

The project-local variant does **not** push; pushing remains a manual user action.

Default commit agent: `General`.

This is **conversation isolation, not worktree isolation**: the child has an ephemeral context/session but intentionally operates on the same `ctx.cwd` workspace so it can commit the real repository changes. The final report is not persisted in the parent session.
